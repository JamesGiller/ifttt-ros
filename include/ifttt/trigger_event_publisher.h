...
public:
    TriggerEventPublisher(std::string src_topic_name, uint32_t src_topic_queue_size,
                          TriggerEventEncoder<SrcMsg> to_trigger_event,
                          std::string event_topic_name, uint32_t event_topic_queue_size) :
            nh_(),
            to_trigger_event_(to_trigger_event)
    {
        event_publisher_ = nh_.advertise<TriggerEvent>(event_topic_name, event_topic_queue_size);

        src_topic_subscriber_ = nh_.subscribe(src_topic_name, src_topic_queue_size,
                                              &TriggerEventPublisher<SrcMsg>::srcTopicCallback_, this);
    }
...
private:
    ...
    void srcTopicCallback_(const typename SrcMsg::ConstPtr & msg)
    {
        TriggerEvent event = to_trigger_event_(*msg);
